...
function Verb-Noun {
    ...
    if ($PSCmdlet.ShouldProcess($Target, $Operation)) {
        ...
        # Implementation
        Write-ToLog -Message "Operation completed" -Level SUCCESS
...
- **Write-ToLog.ps1** - Enterprise logging (thread-safe, rotation, redaction, PSStyle)
...
if ($existing) {
    Write-ToLog -Message "Resource '$Name' already exists" -Level INFO
...
Write-ToLog -Message "Resource '$Name' created" -Level SUCCESS
...
### 5. Enterprise Logging (Write-ToLog.ps1 Pattern)

**All significant operations must be logged:**
```powershell
Write-ToLog -Message "Starting operation: $Operation" -Level INFO
Write-ToLog -Message "Warning: Condition detected" -Level WARN
Write-ToLog -Message "Operation completed successfully" -Level SUCCESS
Write-ToLog -Message "Operation failed: $($_.Exception.Message)" -Level ERROR
```

**Write-ToLog Features (Reference Implementation):**
...
Write-ToLog -Message "Auto-selected $Count of $($items.Count) items: $($info -join ', ')" -Level INFO
```
...
Write-ToLog -Message "Metrics - Total: $totalCount, Processed: $processedCount, Failed: $failedCount, Success Rate: $successRate%" -Level INFO
```
...
- **Sensitive data redaction**: Use Write-ToLog's automatic redaction
...
- [ ] Logging uses Write-ToLog pattern
