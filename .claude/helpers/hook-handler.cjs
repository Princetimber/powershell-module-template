...
const handlers = {
  ...
  'session-restore': async () => {
    ...
    //
    // This project's CLAUDE.md prohibits background network calls without
    // explicit documentation, so both promo-content refreshes below are
    // opt-in (RUFLO_ENABLE_NETWORK_REFRESH=1) rather than default-on.
    if (/^(1|true|yes|on)$/i.test(String(process.env.RUFLO_ENABLE_NETWORK_REFRESH || ''))) {
